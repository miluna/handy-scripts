...
def get_deleted_branches(branch_stdout: str) -> list[str]:
    ...
    for line in branch_stdout.splitlines():
        ...
        if eng_deleted_branch_pattern.match(line) or esp_deleted_branch_pattern.match(line):
...
def main():
    ...
    deleted_branches = subprocess.run(branch_list_verbose,
    ...
    deleted_remote_branches = subprocess.run(clean_remote_deleted_branches, stdout=subprocess.PIPE)
    logger.info(
